...
function evaluarParidad() {
    ...
    if (noValido) {
        // Esta condición se cumple si alguno de las 3 operaciones resulta en verdadero.
...
function validarNumero(promptNum) {
    // La función recibe un valor y devuelve un booleano true/false si el valor es un número entero o no.
    const valido = !(promptNum === null || promptNum === '' || isNaN(promptNum));
    // Esta condición se cumple si alguna de las 3 operaciones resulta en verdadero. Y luego el resultado es negado o invertido.
    // const valido = promptNum !== null && promptNum !== '' && !isNaN(promptNum);
    // Esta condición se cumple si el resultado de todas las operaciones es verdadero.
    return valido;
}
...
function evaluar10Numeros() {
    ...
    for (let contador = 0; contador < 10; contador++) {
        ...
        do {
            // Haremos una validación con un bucle do-while para pedir un número al usuario, hasta que ingrese uno válido.
            numero = prompt('Ingrese un numero entero');
            valido = validarNumero(numero);
            // La validación de números en un prompt() se vuelve frecuente, por tanto creamos una función para reutilizarla.

            if (numero === null) {
                console.log('Se canceló la ejecución 🥺');
                return;
                // Si el usuario presiona cancelar, con return finalizamos prematuramente la ejecución de la función.
            }
            if (!valido) {
                alert('No ingresó un número válido 😠');
            }
        } while (!valido);
        // Mientras el valor ingresado no sea válido, se repetirá el bucle.
        ...
        if (esNegativo) {
            ...
            // +1 a la al contador de positivos si no es negativo.
        }
        if (esMultiploDe15) {
            cantidadDeMultiplosDe15 += 1;
            // +1 al contador de múltiplos si se cumple la condición.
        }
        if (numeroEntero % 2 === 0) {
            acumuladorDePares += numeroEntero;
